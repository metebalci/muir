...
| `quux-promh.mcr` | **QUUX's boot PROM, version 1000, for contracts Q2, Q3 and Q8**, not MIT's: MIT's `promh.text` (version 9) as muir-sys changed it for QUUX --- any PDL width, QUUX's 64 level-2 blocks, block-disk only, and since muir-sys commit `8e20b4c` nothing saved: its buffer is physical page 3, the microcode's main-memory section is loaded last over it, and it writes no block of the disk. Still finds the microcode through MIT's `LABL` label in block 0. Assembled at control store `36000` by muir-sys's builder and handed over as `run/prom-1000-q8`, built from `8e20b4c`'s `promh.text` and `qwmcr.lisp`; copied here unchanged. The file is in **partition order** (contract Q8): MIT's `.mcr` with the two 16-bit halves of every 32-bit word swapped, 139 whole blocks of 1024 bytes. The assembler writes the section from 0; the code is at `36000`-`36554` and nothing is below `36000`; `36000` is `JUMP GO`, `GO` at `36043`, after the new halts `ERROR-TWO-MAIN-MEM-SECTIONS` at `36040` and `ERROR-BUFFER-NOT-LOADED` at `36042`. No `766012` write: error stop through the register page's word 102. SHA-256 `fb7ce53ba18688ef7638c42d7089f6422a29f4bd86d467f059080798e843e7a1`. `tests/quux_prom.rs` reads it from `36000` in partition order and refuses it in MIT's; holds it through the half swap to MIT's own `mit/sys/ubin/promh.mcr` --- the same sections, dispatch and A memory word for word MIT's, only the program QUUX's; and holds it byte for byte to the hand-over, with the halts and the end where the hand-over's `promh.tbl` and `promh.locs` put them, where `ref/prom-1000-q8` is present. `tests/quux_prom_saves_nothing.rs` holds it to saving nothing; `tests/system_1002.rs` boots System 1002 on it. `--machine quux` loads it; the CADR keeps MIT's |
