...
| `quux-promh.mcr` | **QUUX's boot PROM, version 1000, for contract Q2**, not MIT's: muir-sys's `promh.text` (MIT's version 9 as changed for QUUX: any PDL width, QUUX's 64 level-2 blocks, block-disk only), assembled at control store `36000`, handed over as `run/prom-1000-q2` from muir-sys main `9535fdb` plus uncommitted Q2 work (its source goes in with muir-sys's Q2 microcode). The assembler writes the section from 0; the code is at `36000`-`36551` and nothing is below `36000`. No `766012` write: error stop through the register page's word 102. SHA-256 `d13bbee19280561b36c88ccb4545f1d867a1f7b3d2c127491d420a16a1e7d3b6`. `tests/quux_prom.rs` reads it from `36000`, holds its first word to `JUMP GO` at `36037`, and holds it byte for byte to the hand-over where `ref/prom-1000-q2` is present; `tests/system_1002.rs` boots System 1002 on it. `--machine quux` loads it; the CADR keeps MIT's |
