...
| `quux-promh.mcr` | **QUUX's boot PROM, version 1000**, not MIT's: MIT's `promh.text`, version 9, changed by muir-sys (main `d85b54a`) so that a PDL buffer of any width from 1K up boots --- `FILL-A-LOOP` stops after 2000 words by count rather than on the PDL index wrapping to 0, `CLEAR-PDL-BUFFER` starts from a pointer of all ones, and `CLEAR-LEVEL-2-MAP` clears QUUX's 64 blocks. It boots the CADR as MIT's does. SHA-256 `832480a4b61dcb3b6f246f1fbfed95e4a28a1f15340d883e256dbc1ef9eab71b`. `tests/quux.rs` holds it to `mit/sys/ubin/promh.mcr` word for word up to its first change, at `223`; `tests/system_1001.rs` boots System 1001 on it on the CADR and on QUUX with 1K, 4K and 16K buffers. `--machine quux` loads it; the CADR keeps MIT's |
