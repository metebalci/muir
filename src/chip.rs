...
/// An oscillator on the board: the 74LS124 at REQTIM 0A01, whose first VCO
/// clocks the timeout counter.
///
/// **How the circuit works.** `INT BUSY` rises when a Unibus cycle begins
/// and is inverted at 0B03 onto pin 6, the section's enable, which is active
/// low --- a low there is only worth an inverter if a low is what runs it.
/// Pin 7, the output, goes to pin 11 of the 74LS273 at REQTIM 0B01 with
/// nothing in between, and that is the register's clock.
///
...
///
/// MIT's own figures, from the listing's header and the comment on each
/// entry: a normal NXM timeout at state 5, "10 uSec"; a normal hung timeout
/// at state 12 octal, "20 uSec"; and, in the half the `DEBUG REQUEST ACTIVE`
/// address bit selects, an NXM timeout at state 35 octal, "26 uSec" --- the
/// header calls that one 30, and the table is what was burned. The clock
/// they are counted in is "roughly 2 uSec". [`VCO_PERIOD`] takes it at
/// 2,000, and the edges then fall at the enable and every 2,000 ns after,
/// which walks out as
///
/// | edge | at | |
/// |---|---|---|
/// | 1 | 0 | state 1 |
/// | 5 | 8,000 | state 5, where the PROM raises `PROM NXM TIMEOUT` |
/// | 6 | 10,000 | the register takes it: `NXM TIMEOUT` up, MIT's 10 uSec |
/// | 10 | 18,000 | state 10, `PROM HUNG TIMEOUT` |
/// | 11 | 20,000 | `HUNG TIMEOUT` up, MIT's 20 uSec |
///
/// so both figures land on the nanosecond, and they only both land with the
/// register counted and the first edge at the enable.
/// `a_request_nobody_answers_times_out` in `tests/busint_netlist.rs`
/// measures it on the board and holds it there.
///
/// **The datasheet does not describe this behaviour, and probably is not
/// describing this part.** `sn74124.pdf` is Texas Instruments'
/// SN54S124/SN74S124, first published December 1983 and last revised 2004.
/// It says "while the enable input is high, the output is high", and that
/// the synchronisation gating "ensures that the first output pulse is
/// neither clipped nor extended" --- an output that idles high and whose
/// first low pulse is full width, so its first *rising* edge comes half a
/// period after the enable rather than with it.
///
/// The board was wired in 1978 and its part is an LS, not an S. A sheet
/// revised a quarter of a century later describes what the vendor was
/// selling then, and behaviour at the enable is the sort of thing tightened
/// between revisions: early parts were often specified for the steady state
/// only, with the first edge left to whatever the internal gate and the
/// external capacitor did between them. The wording is about *pulses* being
/// full width, too, which is not quite a statement about the polarity of the
...
/// capacitor: `cadr1/reqtim.drw` carries a `DCAP` whose value reads
/// `100 pf`, in the four-pin `DUMMY4` at B03@01 that `cadr1/busint.wlr`
/// hangs across `VCO CAP1` and `VCO CAP2`, with the range input on pin 3 and
/// the frequency-control input on pin 2 both strapped to +5. What that
/// cannot yet be turned into is a period: no 74LS124 sheet has reached us,
/// and the S part's frequency-versus-Cext curve is not the LS part's. An LS
/// sheet, or a scope on a running board, is what is left; MIT's "roughly
/// 2 uSec" in `cadr1/reqtim.prom` is the figure being followed until then.
struct Oscillator {
    output: NetId,
    ...
    /// second of it.
    period: (u64, u64),
    /// When it last started running, and how many times the output has
    /// toggled since: the k-th toggle is at `origin + toggle_at(period, k)`.
    origin: u64,
    edges: u64,
    /// When the output next toggles, while running.
    next: Option<u64>,
}
...
///
/// The bus interface's `reqtim` 0A01 uses section 1 alone and MIT gives it
/// as "roughly 2 uSec". The disk controller's `dctmot` 0B04 uses both,
...
impl Chip {
    ...
    pub fn save(&self, w: &mut impl std::io::Write) -> std::io::Result<()> {
        ...
        // has toggled since, or not running.
        for o in &self.oscillators {
            w.write_all(&time(o.next.map(|_| o.origin)))?;
    ...
    pub fn load(&mut self, r: &mut impl std::io::Read) -> std::io::Result<()> {
        ...
        if timers {
            ...
            for i in 0..self.oscillators.len() {
                r.read_exact(&mut t)?;
                let origin = Some(u64::from_le_bytes(t)).filter(|&v| v != u64::MAX);
                if !edges {
                    if origin.is_some() {
                ...
                o.origin = origin.unwrap_or(0);
                o.edges = count;
                o.next = origin.map(|at| at + toggle_at(period, count + 1));
            }
    ...
    fn advance_delays(&mut self, now: u64) {
        ...
        for i in 0..self.oscillators.len() {
            ...
            if enable.is_some_and(|e| self.nets[e as usize] != Level::Low) {
                self.oscillators[i].next = None;
                // Held low so that enabling it is a rising edge; see the
                // note on [`Oscillator`] for why the sheet's idle high is
                // not followed here.
                self.set_net(output, Level::Low);
                continue;
            }
            let mut level = self.nets[output as usize];
            if self.oscillators[i].next.is_none() {
                // Starting: the output rises the moment it is enabled.
                self.oscillators[i].origin = now;
                self.oscillators[i].edges = 0;
                level = Level::High;
            }
            let origin = self.oscillators[i].origin;
            ...
            let edges = due;
            self.oscillators[i].edges = edges;
            self.oscillators[i].next = Some(origin + toggle_at(period, edges + 1));
