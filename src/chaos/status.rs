...
//! Chaosnet server has no interface and no cable: none of those can happen to it,
//! so nought is the true count. What it can answer truthfully is its name
//! and which subnet it is on, and it does.
...
impl Status {
    /// Answering for a host of this name. The subnet is the high byte of its
    /// address, as [`Status::on_subnet`] takes it.
    pub fn new(name: &str) -> Status {
        Status { name: name.to_string(), subnet: 6 }
    }
    ...
    /// subnet 6.
    pub fn on_subnet(mut self, subnet: u8) -> Status {
        self.subnet = subnet;
        self
    }
