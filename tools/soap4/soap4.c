// SPDX-FileCopyrightText: 2004 Brad Parker
// SPDX-FileCopyrightText: 2026 Mete Balci
// SPDX-License-Identifier: AGPL-3.0-or-later

/*
 * soap4, as this project carries it.
 *
 ...
 *
 * - `-o netlist` writes the 2004 netlist format that `src/netlist.rs`
 *   reads, restored from the commented-out code still in `soap.c`.
 *   `assign_at_names()` goes with it, reproducing the 2004 script's
 *   convention of naming an unnamed net after the pin at its other end.
 * - A name still carrying a non-printable character after
 *   `fix_signal_name` has it dropped and says so, where cadr4 asserts.
 *   `cadrio/clk60h.drw` has a control character in `POWER LINE ^`.
 * - cadr4's label aliases and its LH/HL/LNLH polarity-marker bodies are
 *   gone.  They give its VHDL a second name for a wire the drawings label
 *   twice; here such a wire is put on one net under MIT's own name by
 *   `examples/reconcile.rs`, and the alias table's fixed 256 entries are
 *   an assert waiting to fire on a board with more of them.
 *
 * One thing has been taken from cadr4 since: the comment-only-name fix in
 * `parse_points`, cadr4 ee39593 ("soap4: a text that is only a comment
 * names no signal", 4 September 2026), applied here in the same shape.
 * It recovered nine pins the reader had been dropping.
 */
...
fix_signal_name(char *s)
{
    ...
    // drop any non-printable character that is still there, and say so:
    // cadrio/clk60h.drw has a control character in 'POWER LINE ^'
    {
        char *q = b;
        p = b;
        while (*p) {
            if ((*p < 32) || (*p > 126)) {
                fprintf(stderr, "\nnon-printable character: %d in '%s', dropped\n", *p, s);
            } else {
                *q++ = *p;
            }
            p++;
...
/*
 * The 2004 netlist format, as data/CADR.netlist is written.
 *
 * One `part` block per body --- a gate, not a package --- naming the body
 * def rather than the DIPTYPE, with the net on each named pin in pin order.
 * `=` separates the pin from the net, so the one net called `A=M` is
 * renamed; a name with a space or a parenthesis in it is single quoted.
 * Both rules are read off data/CADR.netlist, which is what this emitter is
 * checked against: it reproduces every page of the processor board byte for
 * byte, save one where the 2004 extraction itself went wrong.
 */
/*
 * The 2004 reader named an unnamed net after the pin at the *other* end, so
 * one wire carries two names: the wire between ALUC4 2C15 pin 3 and 2C20 pin
 * 13 is `@2C20,p13` where 2C15 touches it and `@2C15,p3` where 2C20 does.
 * `src/netlist.rs` joins them again, and this reproduces the convention so
 * that it can.
 */
static void
assign_at_names(void)
{
    DEBUG("assign_at_names\n");

    assign_anonymous_net_names();

    for (size_t n = 0; n < internal_net_names_count; n++) {
        char *net = internal_net_names[n];

        /* the pin points on this net, in point order */
        struct point_s *on_net[64];
        size_t count = 0;
        for (size_t i = 0; i < points_count && count < LEN(on_net); i++) {
            struct point_s *p = &points[i];
            if (p->id.x == 0 || p->id.y == 0) continue;
            if (p->name != net) continue;
            if (find_body(p->id.y) == NULL) continue;
            on_net[count++] = p;
        }
        if (count < 2) continue;

        /* each end takes the name of a different end */
        char *named[64];
        for (size_t i = 0; i < count; i++) {
            struct point_s *other = NULL;
            for (size_t j = 0; j < count; j++) {
                if (on_net[j]->id.y == on_net[i]->id.y &&
                    on_net[j]->pin_name == on_net[i]->pin_name) continue;
                other = on_net[j];
                break;
            }
            if (other == NULL) { named[i] = on_net[i]->name; continue; }
            char b[64];
            snprintf(b, sizeof(b), "@%s,p%u",
                     find_body(other->id.y)->refdes, other->pin_name);
            named[i] = managed_strdup(b);
        }
        for (size_t i = 0; i < count; i++)
            on_net[i]->name = named[i];
    }
}

static char *
netlist_name(char *name)
{
    static char b[512];

    if (strcmp(name, "A=M") == 0) return "AEQM";
    if (strpbrk(name, " ()") == NULL) return name;
    snprintf(b, sizeof(b), "'%s'", name);
    return b;
}

static void
dump_netlist(const char *page_name)
{
    /* soap never filled this in, and the 2004 file has it empty */
    printf("# extracted from SUDS file \n");
    printf("# bodies 0, points 0\n");
    printf("# SUDS version %lu\n", (unsigned long)header->version);
    printf("# nomemclature type %s\n", header->nomenclature_type);
    printf("# board type %s\n", header->board_type);
    printf("# draw by: %s\n", trailer->drawn_by ? trailer->drawn_by : "");
    printf("# title 1: %s\n", trailer->title_line_1 ? trailer->title_line_1 : "");
    if (trailer->title_line_2 != NULL && trailer->title_line_2[0] != 0)
        printf("# title 2: %s\n", trailer->title_line_2);
    printf("\n");

    char *upper = managed_strdup((char *)page_name);
    strupp(upper);
    printf("page %s\n", upper);
    printf("\n");
    ...
    for (size_t i = 0; i < bodies_count; i++) {
        ...
        if (should_skip_body(b)) continue;
        if (b->name_of_body_def == NULL || b->name_of_body_def[0] == 0) continue;
        if (strcmp(b->name_of_body_def, "TABLE") == 0) continue;
        if (strcmp(b->name_of_body_def, "BYPASS") == 0) continue;
        if (strcmp(b->refdes, "0@00") == 0) continue;

        struct body_def_s *bd = find_body_def(b->name_of_body_def);
        if (bd == NULL) continue;

        /* pin number to net, in pin order */
        char *nets[MAX_BODY_DEF_PINS];
        size_t highest = 0;
        memset(nets, 0, sizeof(nets));
        for (size_t j = 0; j < bd->pin_count; j++) {
            struct point_s *pt = find_point(bd->pins[j].id, b->id);
            if (pt == NULL || pt->name == NULL || pt->name[0] == 0) continue;
            if (pt->pin_name >= MAX_BODY_DEF_PINS) continue;
            nets[pt->pin_name] = pt->name;
            if (pt->pin_name > highest) highest = pt->pin_name;
        }

        printf("part %s,%s\n", b->refdes, b->name_of_body_def);
        printf("(\n");
        for (size_t j = 1; j <= highest; j++)
            if (nets[j])
                printf("p%zu=%s\n", j, netlist_name(nets[j]));
        printf(")\n");
        printf("\n");
    }
}
...
usage(char *argv[])
{
    ...
    fprintf(stderr, "-o netlist  dump the 2004 netlist format (stdout)\n");
...
int main(int argc, char *argv[])
{
		...
		switch (c) {
            ...
            case 'o':
                ...
                if (strcmp(dump_format, "raw") == 0) {
                    ...
                    dump_vhdl_flag = 1;
                } else if (strcmp(dump_format, "netlist") == 0) {
                    dump_netlist_flag = 1;
    ...
    if (dump_raw_flag > 0) {
        ...
        if (dump_netlist_flag) {
            DEBUG("preparing to dump netlist\n");
            // No apply_hacks(): its renames invent unique reference
            // designators where three of the drawings genuinely repeat one,
            // and the netlist is meant to be what the drawing says.
            // `src/netlist.rs` does not key parts by designator.
            assign_same_ids_of_points();
            propagate_net_names();
            assign_at_names();
            dump_netlist(page_name);
        }
