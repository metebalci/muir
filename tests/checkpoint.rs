...
#[test]
fn the_format_is_version_11_and_another_version_is_refused() {
    assert_eq!(checkpoint::VERSION, 11, "a new version needs its own tests");
    ...
    assert_eq!(&good[at..at + 4], 11u32.to_le_bytes());
    for other in [1u32, 2, 3, 4, 5, 6, 7, 8, 9, 10, u32::MAX] {
        ...
        assert!(err.contains("reads 11"), "{err}");
    }
    std::fs::remove_dir_all(&dir).ok();
}
