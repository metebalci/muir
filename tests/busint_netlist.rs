...
fn a_request_nobody_answers_times_out() {
    ...
    // takes the request and enables the oscillator's output is half a
    // period on. The 74LS124 has been running since power-on; its output
    // is high until the first fall of that clock after the enable, and
    // the sixth rising edge from there registers the NXM bit: `TIMEOUT_NS`
    // after the first, as `src/busint.rs` counts it for `rtl`.
    assert_eq!(
        t0 + acked,
        muir::busint::nxm_timeout_at(t0 + Board::HALF_NS),
        "when the NXM timeout came"
    );
}

/// **The timeout clock runs free, and its output idles high.**
///
/// The 74LS124 at REQTIM 0A01 is the LS part, and TI's sheet for it ---
/// *The TTL Data Book for Design Engineers*, 2nd edition, 1976, pages
/// 7-123 to 7-125 --- says three things the S part's sheet does not: that
/// "the internal oscillator of the 'LS124 runs continuously even while the
/// output is disabled"; that "while the enable input is high, the output
/// is high"; and that the first edge after the enable is a fall, coming
/// "30 ns plus up to one period" later "depending upon the timing of the
/// enable pulse with respect to the signal generated by the internal
/// oscillator". So where the NXM timeout lands depends on the phase of a
/// clock that started at power-on, not at the grant.
///
/// Requests are made at five phases of that clock, on a fresh board each
/// time, and on every one: the output is high while `INT BUSY` is down;
/// its first edge after the enable is a fall, when [`muir::chip::gated_fall`]
/// says; `NXM TIMEOUT` rises when [`muir::busint::nxm_timeout_at`] says;
/// and every one of those instants is a rising edge of one clock counted
/// from power-on --- a multiple of the period --- while the wait from the
/// grant varies between five and a half periods and six and a half.
#[test]
fn the_timeout_clock_runs_free_and_idles_high() {
    use muir::part::Level;

    let n = busint();
    let signals = support::wire_list(&n, &["cadr1", "busint.wlr"]);
    let (period, over) = muir::chip::VCO_PERIOD;
    assert_eq!(over, 1, "a whole number of nanoseconds");
    let mut waits = Vec::new();
    for offset in [0u64, 200, 400, 600, 800] {
        let mut b = boundary(&n, &signals);
        let start = b.now + offset;
        b.run(start);
        let vco = b.id("@0A01,p7");
        assert_eq!(b.c.net(vco), Level::High, "the output idles high, at {start} ns");
        let memrq = b.id("-MEMRQ");
        b.c.drive(memrq, Level::Low);
        let (mut enabled_at, mut first_edge, mut timed_out) = (None, None, None);
        ...
        while b.now < start + 10_000 {
            ...
            let now_out = b.c.net(vco);
            if enabled_at.is_some() && now_out != out && first_edge.is_none() {
                first_edge = Some((b.now, now_out));
            }
            let now_nxm = b.level("NXM TIMEOUT");
            if now_nxm == Level::High && nxm != Level::High {
                timed_out.get_or_insert(b.now);
            }
            (busy, out, nxm) = (now_busy, now_out, now_nxm);
        }
        let enabled_at = enabled_at.expect("INT BUSY never rose");
        let (fell_at, fell_to) = first_edge.expect("the output never moved");
        let timed_out = timed_out.expect("NXM TIMEOUT never rose");
        eprintln!(
            "enabled at {enabled_at}: first edge {fell_to:?} at {fell_at}, NXM TIMEOUT at \
             {timed_out}, {} ns after the enable",
            timed_out - enabled_at
        );
        assert_eq!(enabled_at, start + Board::HALF_NS, "the grant at the master clock's edge");
        assert_eq!(
            (fell_at, fell_to),
            (muir::chip::gated_fall(muir::chip::VCO_PERIOD, enabled_at), Level::Low)
        );
        assert_eq!(timed_out, muir::busint::nxm_timeout_at(enabled_at));
        assert_eq!(timed_out % period, 0, "a rising edge of the clock counted from power-on");
        waits.push(timed_out - enabled_at);
    }
    assert!(
        waits.iter().all(|&w| w > 5 * period + period / 2 && w <= 6 * period + period / 2),
        "{waits:?}"
    );
    assert!(
        waits.iter().any(|&w| w < 6 * period) && waits.iter().any(|&w| w > 6 * period),
        "{waits:?}"
    );
}
