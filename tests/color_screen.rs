...
//! sync going and writes its color map.
//!
...
///   first word of the color buffer with the error stop off and reads it
...
fn the_cold_boot_sets_up_the_color_tv_and_writes_its_map() {
    ...
    let color = e.machine().color_tv.as_ref().expect("the board is on the backplane");
    eprintln!(
        "at the prompt after {ran}: color mode {:o}, register 3 {:o}, {} sync words loaded",
        color.mode(),
        color.sync.enable,
        color.sync.words().iter().filter(|&&w| w != 0).count()
    );

    // `(SI:START-SYNC 3 0 36.)`: clock mode 3, black-on-white clear, the
    // interrupt enable clear --- which is why the color board's vertical
    // flag never reaches `-XBUS.INTR`, where `INTRX0` would have nothing
    // to clear it with.
    assert_eq!(color.mode() & tv::mode::CLOCK, 3, "clock mode 3");
    assert_eq!(color.mode() & mode::BOW, 0, "BOW clear");
    assert_eq!(color.mode() & mode::INTERRUPT_ENABLE, 0, "and the interrupt enable clear");
    assert!(color.sync.enabled(), "the sync RAM is in");
    assert_eq!(color.sync.enable & 0o177, 36, "vertical spacing 36, MIT's 'for color'");
    ...
    let timeline = color.timeline().expect("the loaded program makes a frame");
    ...
    // sync RAM empty: `tests/vertical.rs` holds that, and the color board
    // being set up does not set the main one up.
    assert!(!e.machine().tv.sync.enabled(), "the main board is untouched");

    // `R-G-B-COLOR-MAP`: color 0 all off, and color I the `(NTH (\\ I 3)
    ...
    assert_eq!(color.color_map(), &want, "the map R-G-B-COLOR-MAP wrote");

    // And what the monitor is shown, which is the map inverted back: black
    // at 0, then red, green and blue over and over.
    assert_eq!(color.rgb(0), [0, 0, 0], "color 0 is black");
    assert_eq!(color.rgb(1), [0, 255, 0], "1 mod 3 is green");
    assert_eq!(color.rgb(2), [0, 0, 255]);
    assert_eq!(color.rgb(3), [255, 0, 0]);
    assert_eq!(color.rgb(0o17), color.rgb(3), "15 mod 3 is red again");
}
