...
| Bus interface | None. A physical address is answered by the machine directly, and a memory cycle is charged a flat 520 ns, the band's mean wait on `rtl`. | Model, timed: the arbitration, `MEMSTART` to `-LOADMD`, the Unibus and Xbus cycles, and the timeout. | `BUSINT.netlist`, always. | as at left |
